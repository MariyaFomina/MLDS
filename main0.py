...
def integral(info):
    ...
    if calculations == 0:
        ...
        # Делаем фотку и сохраняем ее
        photo(calculations, info.chat.id)
        try:
            # Отправляем фото пользователю
            bot.send_photo(info.chat.id, photo=open(f"photo_{info.chat.id}.png", 'rb'))
            # Удаляет фотку - иначе он долго тупит!!!!
    ...
    if message.text is None:
        ...
        try:
            ...
            if mat.shape[0] <= 10 or mat.shape[1] <= 10:
                ...
                df_t = pd.DataFrame(t)
                # file = df.to_excel('calc_excel.xlsx', sheet_name='Transp matr', header=False, index=False)
            ...
            if mat.shape[0] <= 10 or mat.shape[1] <= 10:
                ...
                try:
                    ...
                    # file = df.to_excel('calc_excel.xlsx', sheet_name='Inv matr', header=False, index=False)
